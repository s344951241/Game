...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderUnityEngineVector2
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<CusEncoding.EncodingType>(translator.PushCusEncodingEncodingType, translator.Get, translator.UpdateCusEncodingEncodingType);
        ...
        int CusEncodingEncodingType_TypeID = -1;
		int CusEncodingEncodingType_EnumRef = -1;
        
        public void PushCusEncodingEncodingType(RealStatePtr L, CusEncoding.EncodingType val)
        {
            if (CusEncodingEncodingType_TypeID == -1)
            {
			    bool is_first;
                CusEncodingEncodingType_TypeID = getTypeId(L, typeof(CusEncoding.EncodingType), out is_first);
				
				if (CusEncodingEncodingType_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(CusEncoding.EncodingType));
				    CusEncodingEncodingType_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, CusEncodingEncodingType_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, CusEncodingEncodingType_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for CusEncoding.EncodingType ,value="+val);
            }
			
			LuaAPI.lua_getref(L, CusEncodingEncodingType_EnumRef);
        ...
        public void Get(RealStatePtr L, int index, out CusEncoding.EncodingType val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != CusEncodingEncodingType_TypeID)
				{
				    throw new Exception("invalid userdata for CusEncoding.EncodingType");
				}
                ...
                {
                    throw new Exception("unpack fail for CusEncoding.EncodingType");
                }
				val = (CusEncoding.EncodingType)e;
                
            }
            else
            {
                val = (CusEncoding.EncodingType)objectCasters.GetCaster(typeof(CusEncoding.EncodingType))(L, index, null);
            }
        }
		
        public void UpdateCusEncodingEncodingType(RealStatePtr L, int index, CusEncoding.EncodingType val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for CusEncoding.EncodingType ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			else if (type == typeof(CusEncoding.EncodingType[]))
			{
			    CusEncoding.EncodingType[] array = obj as CusEncoding.EncodingType[];
				translator.PushCusEncodingEncodingType(L, array[index]);
				return true;
			}
            return false;
		}
