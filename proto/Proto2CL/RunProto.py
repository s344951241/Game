	...
	for root,dirs,files in os.walk(path):
		for file in files:
			if os.path.splitext(file)[1]=='.proto':
				...
				dic[os.path.join(root,file)] = os.path.splitext(file)[0]
	return dic
...
def readProto(path):
	...
	for eachLine in file:
		if 'package' in eachLine:
			theName = eachLine.replace('package','')
			theName = theName.strip().strip(';')
			
	if theName == '':
		paths = path.split('/')
		theName = paths[len(paths)-1].replace('.proto','')
	dic[theName] = lis
	file.close()
	file = open(path,"r+")
	for eachLine in file:
		if 'message' in eachLine:
			theValue = eachLine.replace('message','')
			theValue = theValue.strip().strip('{''}')
			theValue = theValue.strip()
			dic[theName].append(theValue)
			theValue = ''
	file.close()
	return dic

def writeLua(path,context):
	file = open(path,"w+")
	file.write(context)
	file.close()

def getCrc32(strs):
	bys = bytes(strs,encoding='utf-8')
	count = len(bys)
	crc = 0xffffffff
